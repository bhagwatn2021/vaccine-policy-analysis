...
doe <- makeDoe(levels, type = "D", nTrials = 100, minTrials = 100, search = T)
...
models <- estimateModels(
    nbreaks = 10,
    data    = data,
    pars    = c("incentive", "value", "penalty", "accessibility"),
    outcome = "choice",
    obsID   = "obsID"
)
...
ggplot(results) +
  ...
  theme_bw() + 
  labs(
    title = "Partial Factorial Power Analysis",
    x = "Sample size", 
    y = "Standard error", 
    color = "Coefficient"
  )
...
Our analysis emphasized the importance of achieving a large sample size (300+ respondents) and using a full factorial design. A partial factorial design would have unwanted confounded variables. By using a full factorial design, the high variability and unexpected results in some of our coeefficients may be examined through the main and interaction effects. Although a fractional factorial design may have smaller standard error, these errors could also be lower without accounting for confounded variables. Therefore, we decided to continue with a full factorial design.
...
Survey Link: https://pilotanalysis.formr.org/
